...
namespace MoreMountains.TopDownEngine
{
	...
	public class TopDownController3D : TopDownController
	{
		...
		public LayerMask ObstaclesLayerMask = LayerManager.ObstaclesLayerMask; //장애물로 간주할 레이어
		/// the length of the raycasts to cast downwards
		[Tooltip("the length of the raycasts to cast downwards")]
		public float GroundedRaycastLength = 5f; ///플레이어가 바닥에 닿았는지
		...
		public float MinimumGroundedDistance = 0.2f; //0.2보다 크면 땅에 붙어있다고 간주하지 않음. <우리는 점프 없으니까 사용하지 않을 예정>
		...
		public GroundedComputationModes GroundedComputationMode = GroundedComputationModes.Advanced; //사용 x
		...
		public float GroundNormalHeightThreshold = 0.2f; //플레이어가 물체와 충돌한 후 점진적으로 멈추게 됌 <사용 안할것같음>
